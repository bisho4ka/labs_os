...
GameManager::GameManager(const char* filename, std::string name) {
    ...
    cells_alive = 0;  // Инициализируем счетчик живых клеток
        ...
        for (size_t j {0}; j < FIELD_SIZE; ++j) {
            ...
            switch (c) {
                case '#':
                    ...
                    cells_alive++;  // Увеличиваем счетчик живых клеток
                    break;
                default:
...
shot_t GameManager::recieve_attack(size_t x, size_t y) {
    if (self_field[y][x] == CELL_ALIVE) {
        ...
        cells_alive--;  // Уменьшаем количество живых клеток
        return SHOT_HIT;
    }
    return SHOT_MISS;
}
